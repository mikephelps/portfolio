...
export default function ClientCase({ project }: ClientCaseProps) {
  ...
  const handleSelect = (index: number) => {
    ...
    // 0.995, not 1: landing exactly on the boundary between this tab's
    // range and the next would make activeIndex's Math.floor round up to
    // the next tab instead. 0.995 clears that edge case by a comfortable
    // margin while filling the highlight bar all the way in practice —
    // 0.96 still left a visible sliver uncovered at the end of the text.
    const desiredProgress = (index + 0.995) / project.screens.length;
